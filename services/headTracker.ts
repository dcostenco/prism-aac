...
export function loadCalibration(): CalibrationData {
  ...
  try {
    ...
    // Shared NaN-defense: rejects Infinity / NaN / non-number /
    // matching leftX-rightX (would freeze the cursor for an AAC
    // user via division-by-zero in the mapping math).
    return isValidCornerCalibration(parsed) ? parsed : DEFAULT_CALIBRATION;
  } catch { /* use defaults */ }
