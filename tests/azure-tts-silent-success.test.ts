...
describe('decodeAndPlay — fail-fast on suspended AudioContext', () => {
  it('returns false when context is stuck suspended after resume (silent-play guard)', async () => {
    ...
    class SuspendedStub {
      ...
      createBufferSource() { return { buffer: null, onended: null, start() {}, stop() {}, connect: (d: unknown) => d, disconnect() {} }; }
      createGain() { return { gain: { value: 0 }, connect: (d: unknown) => d, disconnect() {} }; }
    }
    (globalThis as unknown as { AudioContext: typeof StubAudioContext }).AudioContext = SuspendedStub as unknown as typeof StubAudioContext;
    vi.resetModules();
    const mod = await loadAzureTTS();
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async () =>
      new Response(new ArrayBuffer(64), { status: 200, headers: { 'content-length': '64' } }),
