...
test.describe('Datadog integration safety', () => {
  ...
  test('app boots within performance budget with Datadog wired in', async ({ page, baseURL }) => {
    const t0 = Date.now();
  ...
  test('no unhandled JS errors from Datadog SDK on boot', async ({ page, baseURL }) => {
    const errors: string[] = [];
    ...
    const ddErrors = errors.filter((e) =>
    ...
    expect(ddErrors, `Datadog SDK errors on boot: ${JSON.stringify(ddErrors)}`).toHaveLength(0);
  });

  test('core AAC flow works — keyboard → predictions → speak (no DD regression)', async ({ page, baseURL }) => {
    await bootClean(page, baseURL);
  ...
  test('toolbar button clicks work with DD tracking wired in', async ({ page, baseURL }) => {
    await bootClean(page, baseURL);
    ...
    const settingsModal = page.locator('[data-testid="settings-modal"]').or(page.getByText(/Settings/i).first());
    await expect(settingsModal).toBeVisible({ timeout: 5000 });
  });

  test('no DD network requests block page when vars are absent', async ({ page, baseURL }) => {
  ...
  test('offline mode — app functions when network drops after boot', async ({ page, baseURL, context }) => {
    await bootClean(page, baseURL);
  ...
  test('DatadogInit component renders nothing visible (zero DOM footprint)', async ({ page, baseURL }) => {
    await bootClean(page, baseURL);
