...
test('typing "goo" surfaces prefix-completion predictions, not static defaults', async ({
  page,
}) => {
  ...
  const predictionTexts = await page
  ...
  expect(
    hasPrefixHit,
    `predictions should include a "goo*" word, got ${JSON.stringify(predictionTexts)}`,
  ).toBe(true);
});
...
test('typing "hel" surfaces help/hello completions', async ({ page }) => {
  ...
  const predictionTexts = await page
    .getByRole("button", { name: /^Predict:/ })
    .allInnerTexts();
  expect(predictionTexts.some((t) => t.toLowerCase().startsWith("hel"))).toBe(
    true,
  );
});
...
test("Settings modal opens and shows Synalux account section", async ({
  page,
}) => {
...
test("AI Chat opens — full panel always visible, Speak key sends (2026-05-10 redesign)", async ({
  page,
}) => {
  ...
  await expect(
    page.locator('[data-testid="ai-chat-panel"][data-state="expanded"]'),
  ).toBeVisible();
...
test("Categories panel opens and shows category buttons", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Help|Talk|Food|Places|People|School/i).first(),
  ).toBeVisible();
});

test("Settings shows real Sign-in button (not a token paste field)", async ({
  page,
}) => {
  await page.getByRole("button", { name: "Settings" }).click();
...
test("shift key has caps-lock long-press affordance with bigger letters", async ({
  page,
}) => {
...
test("AI Chat shows mic button when speech recognition is supported", async ({
  page,
  browserName,
}) => {
  await page.getByRole("button", { name: "AI" }).click();
  ...
  const visible = await mic.isVisible();
  if (visible) {
    await expect(mic).toBeVisible();
  }
});

test("keyboard renders within the viewport at the running resolution", async ({
  page,
}, testInfo) => {
  ...
  expect(
    box,
    `Speak button has no bounding box at ${testInfo.project.name}`,
  ).toBeTruthy();
  if (box && viewport) {
