...
test.describe('MathGrid (Phase 1A)', () => {
  test('mounts with grid lines + active-cell cursor visible', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);

    const lines = await page.locator('[data-testid="math-grid-lines"] line').count();
    expect(lines, 'grid renders both vertical and horizontal lines').toBeGreaterThan(8);
  ...
  test('typing a digit commits a glyph and advances the cursor', async ({ page, baseURL }) => {
    ...
    expect(header, 'cursor advanced to (0,1) and one cell stored').toMatch(/cursor=\(0,1\).*cells=1/);
  ...
  test('Enter drops cursor to the next row, column 0', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
    // Focus the footer (not the reset button — Enter would activate it).
    await page.locator('footer').focus().catch(() => { /* footer may not be focusable; that's fine */ });
  ...
  test('tap inside the canvas moves cursor to that cell', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
