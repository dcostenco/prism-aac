...
test.describe("Comfort Player — E2E with real media", () => {
  ...
  test("shows empty state on first open", async ({ page }) => {
    ...
    await expect(page.locator("text=Record voice messages"))
  ...
  test("upload video file and verify it appears in playlist", async ({
    page,
  }) => {
  ...
  test("upload audio file and verify it appears in playlist", async ({
    page,
  }) => {
    const fileInput = page.locator('input[type="file"]');
  ...
  test("click playlist item starts playback and shows Now Playing", async ({
    page,
  }) => {
    const fileInput = page.locator('input[type="file"]');
    await fileInput.setInputFiles(VIDEO_FILE);
  ...
  test("video element renders with correct attributes when playing", async ({
    page,
  }) => {
    const fileInput = page.locator('input[type="file"]');
    await fileInput.setInputFiles(VIDEO_FILE);
  ...
  test("pause and resume playback", async ({ page }) => {
    ...
    await expect(
      page.locator('button[aria-label="Start playback"]'),
    ).toBeVisible();
  });
  ...
  test("delete item from playlist", async ({ page }) => {
    ...
    // Delete
    await page
      .locator('button[aria-label="Delete test-comfort-video"]')
      .click();
    await page.waitForTimeout(500);
  ...
  test("fullscreen mode shows media", async ({ page }) => {
    ...
    await expect(
      page.locator('button[aria-label="Exit fullscreen"]'),
    ).toBeVisible();
  });
