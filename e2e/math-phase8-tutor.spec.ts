...
test.describe("Phase 8 — History tab", () => {
  test("chip mounts the history keyboard with eras + centuries + periods + events (en default)", async ({
    page,
    baseURL,
  }) => {
    ...
    // Universal rows.
    await expect(
      page.locator('[data-testid="math-history-eras-before-common-era"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-eras-common-era"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-centuries-twentieth"]'),
    ).toBeVisible();
    // World periods always visible.
    await expect(
      page.locator('[data-testid="math-history-periods-stone-age"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-periods-medieval"]'),
    ).toBeVisible();
    // English-locale periods + events.
    await expect(
      page.locator('[data-testid="math-history-periods-renaissance"]'),
    ).toBeVisible();
    await expect(
  ...
  test("typing a universal year + CE commits 2 cells", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
  ...
  test("switching language to ro surfaces Romanian events + periods, hides English-only ones", async ({
    ...
    await expect(
      page.locator('[data-testid="math-history-events-stephen-the-great"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-events-romanian-revolution"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-periods-phanariot"]'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="math-history-periods-victorian"]'),
    ).toHaveCount(0);
  ...
  test("zh locale exposes Chinese dynastic events + period names", async ({
    ...
    await expect(
      page.locator('[data-testid="math-history-keyboard"]'),
    ).toHaveAttribute("data-locale", "zh");
    await expect(
      page.locator('[data-testid="math-history-events-tang-dynasty"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-events-prc-founding"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-history-periods-ming-dynasty-period"]'),
    ).toBeVisible();
  });
});

test.describe("Phase 8 — Language Arts tab", () => {
  test("chip mounts the LA keyboard with parts-of-speech + sentence types + punctuation + citation", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="math-language-arts-keyboard"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-la-pos-noun"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-la-pos-verb"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-la-pos-adjective"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-la-sentence-interrogative"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-la-cite-mla"]'),
    ).toBeVisible();
  });

  test("typing N V ADJ commits 3 part-of-speech tags", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
...
test.describe("Phase 8 — full curriculum domain switch", () => {
  test("all 9 non-math domains route distinct prompts and tag the overlay correctly", async ({
    ...
    const cases: Array<{ tab: string; tile: string; domain: string }> = [
      {
        tab: "chemistry",
        tile: "math-chemistry-elements-hydrogen",
        domain: "chemistry",
      },
      { tab: "physics", tile: "math-physics-greek-alpha", domain: "physics" },
      {
        tab: "programming-python",
        tile: "math-python-kw-def",
        domain: "programming-python",
      },
      {
        tab: "programming-java",
        tile: "math-java-kw-public",
      ...
      {
        tab: "history",
        tile: "math-history-eras-common-era",
        domain: "history",
      },
      {
        tab: "language-arts",
        tile: "math-la-pos-noun",
