...
test.describe("MathDocsTool (Phase 5B)", () => {
  test("Save + Open buttons render in the math header", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="math-docs-open-toggle"]'),
    ).toBeVisible();
  });

  test('Saving an empty grid shows a "nothing to save" toast', async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
  ...
  test("Save → toast confirms; Open shows the saved doc; tap reloads it", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
  ...
  test("Delete button removes a saved doc from the list", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
    ...
    const deleteBtn = page
      .locator('[data-testid^="math-docs-delete-"]')
      .first();
