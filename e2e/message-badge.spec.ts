...
test.describe('Message badge → inbox flow', () => {
  ...
  test('badge click opens Schedule (inbox) when unread messages exist', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (store.addIncomingMessage) {
        store.addIncomingMessage({ sender: 'Mom', text: 'Are you OK?', provider: 'sms', receivedAt: Date.now() });
        store.addIncomingMessage({ sender: 'Dad', text: 'Coming to pick you up', provider: 'sms', receivedAt: Date.now() });
        store.addIncomingMessage({ sender: 'Teacher', text: 'Great job today!', provider: 'mail', receivedAt: Date.now() });
      }
    });
    ...
    if (await badge.isVisible()) {
      ...
      expect(await schedulePanel.isVisible() || await page.locator('text=Inbox').isVisible()).toBeTruthy();
    }
  });

  test('badge click opens AAC Chat (send) when no unread messages', async ({ page }) => {
    ...
    if (!hasBadge) {
      ...
      const chatBtn = page.locator('button[aria-label*="Send a message"], button[aria-label*="aac_chat"]');
  ...
  test('clicking contact in inbox shows message and reply option', async ({ page }) => {
