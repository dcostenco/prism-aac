...
test.describe('MathGrid Phase 5D — two-hit magnify', () => {
  test('off (default) → first tap commits instantly, no armed visual', async ({ page, baseURL }) => {
  ...
  test('on → first tap ARMS (data-armed=1, scaled, NO commit)', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
  ...
  test('on → second tap COMMITS the glyph and disarms', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    await setTwoHit(page, true);
  ...
  test('on → 2 s of inactivity AUTO-DISARMS', async ({ page, baseURL }) => {
    ...
    await expect(key, 'auto-disarmed after 2s').toHaveAttribute('data-armed', '0');
    // No commit either.
    const header = await page.locator('header').first().innerText();
    expect(header, 'no commit during the wait').toMatch(/cells=0/);
  });

  test('on → arming a DIFFERENT key clears the prior armed key visually', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    await setTwoHit(page, true);
  ...
  test('on + dwell=400 → second tap still requires the dwell hold', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    await setTwoHit(page, true);
    await page.evaluate(() => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
