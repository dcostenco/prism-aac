...
const KEYBOARDS: Array<{ catId: string; kbId: string; sampleKey: string; sampleGlyph: string }> = [
  { catId: 'misc-math',     kbId: 'math-misc-keyboard',          sampleKey: 'math-misc-keyboard-element-of', sampleGlyph: '∈' },
  { catId: 'time-distance', kbId: 'math-time-distance-keyboard', sampleKey: 'math-time-distance-keyboard-second', sampleGlyph: 's' },
  { catId: 'weight',        kbId: 'math-weight-keyboard',         sampleKey: 'math-weight-keyboard-kilogram', sampleGlyph: 'kg' },
  { catId: 'volume',        kbId: 'math-volume-keyboard',         sampleKey: 'math-volume-keyboard-liter', sampleGlyph: 'L' },
  { catId: 'geom',          kbId: 'math-geom-keyboard',           sampleKey: 'math-geom-keyboard-triangle', sampleGlyph: '△' },
  { catId: 'money',         kbId: 'math-money-keyboard',          sampleKey: 'math-money-keyboard-dollar', sampleGlyph: '$' },
];
...
test.describe('MathGrid Phase 2C — content keyboards', () => {
  for (const { catId, kbId, sampleKey, sampleGlyph } of KEYBOARDS) {
    ...
    test(`${catId} sample key commits glyph "${sampleGlyph}"`, async ({ page, baseURL }) => {
      await gotoDev(page, baseURL);
      await page.locator(`[data-testid="math-category-${catId}"]`).click();
      await page.waitForTimeout(120);
      ...
      expect(header, `${catId} commit advanced to (0,1) and 1 cell stored`).toMatch(/cursor=\(0,1\).*cells=1/);
      const glyphs = await page.locator('[data-testid="math-grid-glyphs"] text').count();
      expect(glyphs).toBeGreaterThanOrEqual(1);
    });
  }

  test('every key on every Phase 2C keyboard meets the 44px tap-target floor', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
