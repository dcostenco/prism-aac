...
async function openMath(page: Page) {
  ...
  await page.evaluate(() => {
    try {
      ...
      cur.state = cur.state || {};
      cur.state.toolbarConfig = cur.state.toolbarConfig || { order: [], enabled: {} };
...
test.describe('Phase 6 — math takes over the viewport', () => {
  test('Prediction tiles + qwerty + categories are hidden while math is open', async ({ page, baseURL }) => {
    await bootClean(page, baseURL);
    // Sanity: prediction tiles + qwerty exist BEFORE math opens.
    const predictionsBefore = await page.locator('[aria-label^="Predict:"]').count();
    expect(predictionsBefore, 'predictions visible pre-math').toBeGreaterThan(0);
    expect(await page.locator('button[data-key="Q"]').count()).toBe(1);

    await openMath(page);

    // Predictions row is gone.
    await expect(page.locator('[aria-label^="Predict:"]'), 'no prediction tiles in math mode').toHaveCount(0);
    // Qwerty letter Q is gone (the qwerty is hidden for panels with
    // their own keyboard, which math has).
    await expect(page.locator('button[data-key="Q"]'), 'qwerty hidden in math mode').toHaveCount(0);
  ...
  test('Closing math brings the chrome back', async ({ page, baseURL }) => {
    ...
    await expect(page.locator('[aria-label^="Predict:"]').first()).toBeVisible();
  });

  test('Math canvas owns the full vertical space (no chrome competing)', async ({ page, baseURL }) => {
    await bootClean(page, baseURL);
    await openMath(page);
    ...
    expect(box.height, 'math owns most of the viewport').toBeGreaterThan(vh * 0.75);
  });
});
