...
async function setRomanian(page: Page) {
  await page.evaluate(() => {
    try {
      ...
      cur.state = { ...(cur.state ?? {}), language: 'ro', outputLanguage: 'ro' };
      localStorage.setItem('prism-aac-settings', JSON.stringify(cur));
    } catch { /* */ }
  });
  await page.reload({ waitUntil: 'domcontentloaded' });
  await page.waitForSelector('button:has-text("Q"), button:has-text("Vorbește")', { timeout: 30000 });
}

test.describe('Keyboard L4 viewport fit (real browser, real layout)', () => {
  test('ai-chat panel: every qwerty row stays inside viewport', async ({ page, baseURL }, testInfo) => {
    ...
    const qBox = await page.locator('button[data-key="Q"]').first().boundingBox();
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, `aichat-keyboard-${testInfo.project.name}.png`),
      fullPage: false,
    });
  });
