...
test.describe("MathGrid Phase 2B — decorations", () => {
  test("Adv. Math panel exposes the 5 decoration tools", async ({
    page,
    baseURL,
  }) => {
    ...
    for (const tool of [
      "fraction-box",
      "fraction-to-denominator",
      "long-division",
      "root-bar",
      "summation-line",
    ]) {
      await expect(
        page.locator(`[data-testid="math-tool-${tool}"]`),
      ).toBeVisible();
    }
  });

  test("Open Fraction Box adds a fraction-bar SVG line", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPageOnAdvMath(page, baseURL);
  ...
  test("Long-Division House adds TWO decoration lines (top bar + side tick)", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPageOnAdvMath(page, baseURL);
    ...
    const after = await page
      .locator('[data-testid="math-grid-decorations"] line')
      .count();
    expect(after - before, "two new decoration lines drawn (bar + tick)").toBe(
      2,
    );
  ...
  test("Root Bar adds a single SVG line and leaves cursor in place", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPageOnAdvMath(page, baseURL);
    ...
    expect(afterHeader, "cursor unchanged").toMatch(
      new RegExp(
        `cursor=\\(${beforeCursorMatch[1]},${beforeCursorMatch[2]}\\)`,
      ),
    );
  });

  test("Summation Line toggles on / off when row has filled cells", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPageOnAdvMath(page, baseURL);
  ...
  test("Move-to-Denominator drops cursor from numerator row to denominator row", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPageOnAdvMath(page, baseURL);
    ...
    await page
      .locator('[data-testid="math-tool-fraction-to-denominator"]')
      .click();
    await page.waitForTimeout(120);
    const header = await page.locator("header").first().innerText();
    expect(header).toMatch(/cursor=\(1,0\)/);
