...
test.describe('MathGrid Phase 1C — Smart Return', () => {
  test('Smart Return aligns with leftmost filled column when row is empty-anchored at 0', async ({ page, baseURL }) => {
  ...
  test('Smart Return aligns with leftmost filled column when row is offset', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
    ...
    await page.waitForTimeout(120);
    const header = await page.locator('header').first().innerText();
    expect(header, 'aligned to leftmost filled col on row 0 (=4)').toMatch(/cursor=\(1,4\)/);
  });

  test('Right Return aligns one past the rightmost filled column', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
    const svg = await page.locator('[data-testid="math-grid-svg"]').boundingBox();
    if (!svg) throw new Error('svg missing');
    await page.mouse.click(svg.x + 4 * 56 + 28, svg.y + 0 * 56 + 28);
  ...
  test('Vertical addition: type 23, Return, type +45 — cells stack', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
    const svg = await page.locator('[data-testid="math-grid-svg"]').boundingBox();
    if (!svg) throw new Error('svg missing');
  ...
  test('Smart Return on empty row drops cursor to (r+1, 0)', async ({ page, baseURL }) => {
    await gotoDevPage(page, baseURL);
