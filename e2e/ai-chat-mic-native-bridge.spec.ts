...
test.describe('AI Chat mic — native bridge JS path', () => {
  ...
  test('tap mic → bridge.startVoice called, listening state set', async ({ page }) => {
    ...
    const before = await page.evaluate(() => (window as unknown as { __bridgeCalls: unknown[] }).__bridgeCalls.length);
    expect(before).toBe(0);

    await mic.click();

    // After tap: bridge.startVoice was called
    const calls = await page.evaluate(() => (window as unknown as { __bridgeCalls: Array<{action: string}> }).__bridgeCalls);
  ...
  test('engine error from native → onError → mic resets, no stuck state', async ({ page }) => {
    await page.goto('/prism-aac');
    await page.locator('button[aria-label="Settings"]').first().waitFor({ timeout: 15_000 });
    await page.locator('button[aria-label="AI"]').first().click();
    await page.locator('[data-testid="ai-chat-panel"]').waitFor();
    ...
    await page.evaluate(() => {
      const fn = (window as unknown as { prismNativeSpeechError?: (e: unknown) => void }).prismNativeSpeechError;
      fn?.('denied');
    });

    // Mic should reset to non-listening state without crashing the panel
    await expect(mic).toHaveAttribute('aria-pressed', 'false', { timeout: 3_000 });
  });

  test('interim then silence → last interim is submitted (no final required)', async ({ page }) => {
    await page.goto('/prism-aac');
    await page.locator('button[aria-label="Settings"]').first().waitFor({ timeout: 15_000 });
    await page.locator('button[aria-label="AI"]').first().click();
    await page.locator('[data-testid="ai-chat-panel"]').waitFor();
    ...
    await page.evaluate(() => {
      const fn = (window as unknown as { prismNativeSpeechResult?: (r: unknown) => void }).prismNativeSpeechResult;
    ...
    const stopCalls = await page.evaluate(() =>
      (window as unknown as { __bridgeCalls: Array<{action: string}> }).__bridgeCalls
        .filter((c) => c.action === 'stopVoice').length,
    );
