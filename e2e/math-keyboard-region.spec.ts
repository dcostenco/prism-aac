...
test.describe("MathKeyboardRegion (Phase 2A)", () => {
  test("region mounts with all 9 category chips visible", async ({
    page,
    baseURL,
  }) => {
    ...
    for (const cat of [
      ...
      await expect(
        page.locator(`[data-testid="math-category-${cat}"]`),
      ).toBeVisible();
    }
  });

  test('default active category is "main" on first load', async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("tapping Adv. Math chip swaps in the adv-math panel", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="math-adv-math-keyboard"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-main-keyboard"]'),
    ).toHaveCount(0);
    ...
    await expect(
      page.locator('[data-testid="math-key-adv-square-root"]'),
    ).toBeVisible();
  });

  test("tapping Letters chip swaps in the letters panel; toggle switches a-p ↔ q-z", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    ...
    await page.waitForTimeout(120);
    await expect(
      page.locator('[data-testid="math-letters-keyboard"]'),
    ).toBeVisible();
  ...
  test("every category swaps to its own keyboard panel (no placeholders left)", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("Adv. Math: tapping π commits the π glyph in the active cell", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("canvas height is STABLE when swapping between categories (no canvas reflow)", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    ...
    for (const cat of ["adv-math", "letters", "misc-math", "main"]) {
      ...
      expect(
        ...
        `canvas height stable across swap to ${cat}`,
      ).toBeLessThanOrEqual(2);
    }
  });

  test("every category chip meets the 44px tap-target floor", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    ...
    expect(
      chips.length,
      "all 19 chips (9 math + 4 Phase-6 + 4 Phase-7 + 2 Phase-8 humanities)",
    ).toBe(19);
