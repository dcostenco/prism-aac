...
test.describe('MathGrid Phase 5A — hold-time dwell', () => {
  test('with mathHoldTimeMs=0 (default) keys commit instantly on press', async ({ page, baseURL }) => {
    ...
    const holdMs = await page.locator('[data-testid="math-key-5"]').getAttribute('data-hold-ms');
    expect(holdMs).toBe('0');
  });

  test('every digit key has a data-hold-ms attribute (DwellButton wrapper applied)', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    for (const d of ['1', '5', '0']) {
      const attr = await page.locator(`[data-testid="math-key-${d}"]`).getAttribute('data-hold-ms');
      expect(attr, `digit ${d} wrapped in DwellButton`).not.toBeNull();
    }
  });

  test('with mathHoldTimeMs=600, releasing early CANCELS the commit', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    ...
    const ringDuringPress = await page.locator('[data-testid="dwell-ring"]').count();
    expect(ringDuringPress, 'dwell ring rendered during dwell').toBeGreaterThanOrEqual(1);
    await page.mouse.up();
    await page.waitForTimeout(120);
  ...
  test('with mathHoldTimeMs=400, holding past the threshold COMMITS the glyph', async ({ page, baseURL }) => {
    await gotoDev(page, baseURL);
    await page.evaluate(() => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const stores = (window as any).__devMathStores;
