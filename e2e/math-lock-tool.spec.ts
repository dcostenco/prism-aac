...
test.describe("MathLockTool (Phase 3A)", () => {
  test("Lock + Unlock buttons render in idle mode", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="math-lock-unlock"]'),
    ).toBeVisible();
    // Status banner is HIDDEN in idle.
    await expect(page.locator('[data-testid="math-lock-status"]')).toHaveCount(
      0,
    );
  });

  test("Tapping Lock shows status; tapping two cells locks region (green tint persists)", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="math-lock-tool"]'),
    ).toHaveAttribute("data-mode", "lock-start");
    await expect(
      page.locator('[data-testid="math-lock-status"]'),
    ...
    await expect(
      page.locator('[data-testid="math-lock-tool"]'),
    ).toHaveAttribute("data-mode", "lock-end");
  ...
  test("Cancel button exits lock mode without committing a region", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
  ...
  test("Unlock flow lifts the lock from a region", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
