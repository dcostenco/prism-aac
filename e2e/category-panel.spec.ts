...
test("HOME grid tiles have visible text labels", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Hello|Goodbye|Thank you|Please/i).first(),
  ).toBeVisible();
});
...
test("HOME bottom tab strip shows category tabs", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Actions|People|Places|Animals|Colors/i).first(),
  ).toBeVisible();
});

test("clicking a category tile navigates to its detail view", async ({
  page,
}) => {
...
test("category detail view tiles have text and are clickable", async ({
  page,
}) => {
  // Navigate to Core Verbs via the bottom tab
  await page
    .getByText(/Core Verbs/i)
    .first()
    .click();
  // Text labels should be visible (Want, Like, Have, etc.)
  await expect(page.getByText(/Want|Like|Have|Go|Do|Can/i).first()).toBeVisible(
    { timeout: 5000 },
  );
});

test("clicking a phrase tile appends text to the message bar", async ({
  page,
}) => {
  ...
  await expect(page.locator('[aria-label="Message text"]')).toContainText(
    /Hello/i,
    { timeout: 3000 },
  );
});
...
test("sidebar Search opens search input", async ({ page }) => {
  ...
  await expect(page.getByPlaceholder(/Search all vocabulary/i)).toBeVisible({
    timeout: 3000,
  });
});
...
test("Time category shows subcategory folders", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Clock Time|Days|Months|Dates|Seasons/i).first(),
