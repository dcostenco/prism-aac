...
function findStores(win: Window): Array<{ getState: () => StoreState }> {
  return (Object.values(win) as unknown[]).filter(
    (v): v is { getState: () => StoreState } =>
      typeof v === 'object' && v !== null && typeof (v as Record<string, unknown>).getState === 'function',
  );
}
...
async function injectHelpers(page: Page) {
  await page.evaluate(() => {
    const stores = (Object.values(window) as unknown[]).filter(
      (v): v is { getState: () => Record<string, unknown>; setState?: (p: unknown) => void } =>
    ...
    // setText helper
    (window as Window & { __prism_set_text?: (t: string) => void }).__prism_set_text = (text: string) => {
      for (const s of stores) {
        const state = s.getState();
    ...
    const dmitriSms = {
      ...
      recipientId: '+19055551234', order: 1,   // placeholder E.164
    };
    for (const s of stores) {
      const state = s.getState();
...
async function sendViaProvider(page: Page, providerLabel: 'Mail' | 'SMS') {
  ...
  await page.evaluate(() => {
    ...
    for (const v of wins) {
      if (v && typeof v === 'object' && typeof (v as Record<string, unknown>).getState === 'function') {
        const state = (v as { getState: () => Record<string, unknown> }).getState();
  ...
  await page.evaluate((q) => {
    (window as Window & { __prism_set_text?: (t: string) => void }).__prism_set_text?.(q);
  }, SEARCH_QUERY);
  ...
  const dmTile = page.locator('[data-testid^="pred-contact-"]').filter({ hasText: CONTACT_NAME }).first();
  await expect(dmTile).toBeVisible({ timeout: 5_000 });
  await dmTile.click();
  await page.waitForTimeout(500);

  // 5. Compose panel should be active.
  const composePanel = page.locator('[data-testid="aac-chat-panel"][data-state="compose"]');
  ...
  if (!headerText.includes(providerLabel)) {
    const chip = composePanel.locator('button').filter({ hasText: providerLabel }).first();
    if (await chip.isVisible()) {
      await chip.click();
      await page.waitForTimeout(300);
    } else {
      console.log(`[send-to-dmitri] ${providerLabel} chip not visible — contact may not have this provider`);
    }
  }

  console.log(`[send-to-dmitri] header: "${await h2.innerText().catch(() => '')}" for ${providerLabel}`);

  // 7. Type message using the on-screen keyboard keys.
  //    We type 'hi' to keep it minimal — just enough to enable Send.
  const KEY_H = page.locator('button[data-key="H"], button[data-key="h"]').first();
  const KEY_I = page.locator('button[data-key="I"], button[data-key="i"]').first();
  if (await KEY_H.isVisible()) { await KEY_H.click(); await KEY_I.click(); }
  await page.waitForTimeout(300);
  ...
  // Close panel.
  const closeBtn = composePanel.locator('button[aria-label*="Close"], button[aria-label*="close"]').last();
...
test.describe('send to Dmitri Costenco', () => {
  test.beforeEach(async ({ page, baseURL }) => {
    ...
    await page.addInitScript(() => {
      ...
      localStorage.setItem('prism-aac-contacts', JSON.stringify({
        state: { contacts, lastSyncedAt: Date.now() },
        version: 0,
      }));
    });

    await page.goto(baseURL ?? BASE_URL, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  test('SMS to Dmitri Costenco — tier-lock verified for unauthenticated session', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      for (const v of wins) {
        if (v && typeof v === 'object' && typeof (v as Record<string, unknown>).getState === 'function') {
          const state = (v as { getState: () => Record<string, unknown> }).getState();
          if (typeof state?.openAACChat === 'function') { (state.openAACChat as () => void)(); return; }
        }
      }
      const btns = Array.from(document.querySelectorAll('button'));
    ...
    await expect(page.locator('[data-testid="prediction-bar-contacts"]')).toBeVisible({ timeout: 6_000 });
    const dmTile = page.locator('[data-testid^="pred-contact-"]').filter({ hasText: CONTACT_NAME }).first();
    await dmTile.click();
    await page.waitForTimeout(400);
    ...
    const composePanel = page.locator('[data-testid="aac-chat-panel"][data-state="compose"]');
    await expect(composePanel).toBeVisible({ timeout: 5_000 });
    const smsChip = composePanel.locator('button').filter({ hasText: 'SMS' }).first();
    ...
    if (await tierWarning.isVisible({ timeout: 2_000 }).catch(() => false)) {
      console.log('[send-to-dmitri] SMS correctly tier-locked (tier warning visible)');
    } else {
      // If the SMS contact is the primary (not Mail), send button should be disabled.
      const sendBtn = page.locator('[data-testid="aac-chat-send-btn"]');
      await expect(sendBtn).toBeDisabled({ timeout: 3_000 });
      console.log('[send-to-dmitri] SMS correctly tier-locked (Send button disabled for free plan)');
    }
  });
});
