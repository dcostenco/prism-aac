...
test.describe("earth-science workflow", () => {
  ...
  test("light-year conversion — 2 ly ≈ 18 trillion km", async ({
    page,
  }, ti) => {
    await runProblem(page, ti, ["1ly=9km", "2ly=2×9", "2ly=18km"], CATEGORY);
  });
  ...
  test("AU conversion — 3 AU ≈ 450 million km", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["1AU=150km", "3AU=3×150", "3AU=450km"],
      CATEGORY,
    );
  });

  test("plate tectonic rate — 5 cm/year for 200 years", async ({
    page,
  }, ti) => {
    await runProblem(page, ti, ["r=5", "y=200", "d=5×200", "d=1000"], CATEGORY);
  });
