...
test.describe("earth-science grade-9 workflow", () => {
  ...
  test("temperature conversion — 25°C to °F", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["°F=(°C×9÷5)+32", "°F=(25×9÷5)+32", "°F=45+32", "°F=77"],
      CATEGORY,
    );
  });

  test("pressure drop — surface 1013mb, Everest 333mb", async ({
    page,
  }, ti) => {
    await runProblem(page, ti, ["Δ=1013−333", "Δ=680"], CATEGORY);
  });
  ...
  test("geologic age — fossil 65Mya to today", async ({ page }, ti) => {
    ...
    await runProblem(
      page,
      ti,
      ["t0=65", "e=0", "age=65−0", "age=65"],
      CATEGORY,
    );
  });

  test("AU to km — Mars 1.52AU×1.50×10⁸km", async ({ page }, ti) => {
    await runProblem(page, ti, ["d=1.52×1.50×10⁸", "d=2.28×10⁸"], CATEGORY);
  });
});
