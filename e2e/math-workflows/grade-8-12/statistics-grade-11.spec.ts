...
test.describe("statistics grade-11 workflow", () => {
  ...
  test("sample mean — scores 7,8,6,9,10", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["n=5", "x̄=(7+8+6+9+10)÷n", "x̄=40÷5", "x̄=8.0"],
      CATEGORY,
    );
  });
  ...
  test("z-score — exam 85, μ=70, σ=10", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["z=(x−μ)÷σ", "z=(85−70)÷10", "z=15÷10", "z=1.5"],
      CATEGORY,
    );
  });
  ...
  test("hypothesis test — p-value 0.03, α=0.05", async ({ page }, ti) => {
    await runProblem(page, ti, ["H0", "Ha", "p≈0.03", "α=0.05"], CATEGORY);
  });
  ...
  test("binomial — 5 flips, P(exactly 3 heads)", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["n=5", "p=0.5", "C(5,3)=10", "P(=10×0.125×0.25", "P(=0.3125"],
      CATEGORY,
    );
  });
});
