...
test.describe("programming python grade-9 workflow", () => {
  ...
  test("function — return square of x", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["def", "square(x):", "return", "x*x", "square(7)", "49"],
      CATEGORY,
    );
  });
  ...
  test("even/odd checker — n=14", async ({ page }, ti) => {
    ...
    await runProblem(page, ti, ["n=14", "n%2==0", "output=even"], CATEGORY);
  });
  ...
  test("list average — grades [80,90,70,100]", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["total=340", "n=4", "avg=total/n", "avg=85.0"],
      CATEGORY,
    );
  });
  ...
  test("find maximum — max of [12,7,23,19]", async ({ page }, ti) => {
    ...
    await runProblem(page, ti, ["nums=[12,7,23,19]", "val=23"], CATEGORY);
  });
  ...
  test("for loop sum — 1 to 10", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["total=0", "for", "i", "in", "range(1,11):", "total=total+i", "55"],
      CATEGORY,
    );
  });
});
