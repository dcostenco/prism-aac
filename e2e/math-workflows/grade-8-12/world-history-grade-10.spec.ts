...
test.describe("world-history grade-10 workflow", () => {
  ...
  test("date arithmetic — 1453 to 1500", async ({ page }, ti) => {
    ...
    await runProblem(page, ti, ["1500−1453", "47"], CATEGORY);
  });

  test("WWI duration — 1914 to 1918", async ({ page }, ti) => {
    await runProblem(page, ti, ["1918−1914", "4"], CATEGORY);
  });

  test("WWII duration — 1939 to 1945", async ({ page }, ti) => {
    await runProblem(page, ti, ["1945−1939", "6"], CATEGORY);
  });

  test("Cold War span — 1945 to 1989", async ({ page }, ti) => {
    await runProblem(page, ti, ["1989−1945", "44"], CATEGORY);
  });

  test("BCE to CE span — Roman Republic 509BCE to 476CE", async ({
    page,
  }, ti) => {
    await runProblem(page, ti, ["509+476", "985"], CATEGORY);
  });
});
