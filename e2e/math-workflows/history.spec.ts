...
test.describe("history workflow", () => {
  ...
  test("years between 1492 and 1776 = 284", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["s=1492", "e=1776", "y=1776−1492", "y=284"],
      CATEGORY,
    );
  });
  ...
  test("century of 1850 = 19th", async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["y=1850", "c=1850÷100", "c=19", "a=19th"],
      CATEGORY,
    );
  });

  test("century of 50 CE = 1st", async ({ page }, ti) => {
    await runProblem(page, ti, ["y=50CE", "c=1", "a=1st"], CATEGORY);
  });

  test("order three eras: BCE → CE → AD", async ({ page }, ti) => {
    await runProblem(page, ti, ["1st=BCE", "2nd=CE", "3rd=AD"], CATEGORY);
  });
