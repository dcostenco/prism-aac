...
for (const d of "0123456789")
  MAIN[d] = { category: "main", testid: `math-key-${d}` };
...
for (const [g, label] of ADV) {
  ADV_MAP[g] = {
    category: "adv-math",
    testid: `math-key-adv-${label.replace(/ /g, "-")}`,
  };
}
...
for (const [g, label] of ADV_TRIG) {
  if (!ADV_MAP[g])
    ADV_MAP[g] = {
      category: "adv-math",
      testid: `math-adv-trig-${slug(label)}`,
    };
}
for (const [g, label] of ADV_CALC) {
  if (!ADV_MAP[g])
    ADV_MAP[g] = {
      category: "adv-math",
      testid: `math-adv-calc-${slug(label)}`,
    };
}
for (const [g, label] of ADV_SUBSCRIPTS) {
  if (!ADV_MAP[g])
    ADV_MAP[g] = {
      category: "adv-math",
      testid: `math-adv-subscripts-${slug(label)}`,
    };
}
...
const GRIDS: Grid[] = [
  ...
  // Biology — genetics
  {
    ...
    pairs: [
      ...
      ["BB", "big-b homozygous"],
      ["Bb", "big-b heterozygous"],
      ["bb", "big-b homozygous recessive"],
      ["F1", "first generation"],
  ...
  // Language arts — POS
  {
    ...
    pairs: [
      ...
      ["conj.", "conjunction lower"],
      ["art.", "article lower"],
      ["intj.", "interjection lower"],
      ["aux.", "auxiliary lower"],
      ["det.", "determiner lower"],
      ["num.", "numeral lower"],
    ],
  },
...
for (const [g, label] of SHARED_DECOR_PAIRS) {
  if (!ADV_MAP[g])
    ADV_MAP[g] = {
      category: "adv-math",
      testid: `math-adv-decor-${slug(label)}`,
    };
}
...
for (const [g, label] of SHARED_DECOR_PAIRS) {
  if (!MAIN[g])
    MAIN[g] = {
      category: "main",
      testid: `math-main-decor-${label.replace(/ /g, "-")}`,
    };
}
...
function buildProgMap(lang: "python" | "java"): Record<string, KeyRef> {
  const cat: KeyRef["category"] =
    lang === "python" ? "programming-python" : "programming-java";
  ...
  if (lang === "python") {
    for (const bi of PYTHON_BUILTINS_LIST) {
      if (!out[bi])
        out[bi] = { category: cat, testid: `${prefix}-builtin-${bi}` };
    }
    for (const [g, label] of PYTHON_EXTRAS_LIST) {
      if (!out[g])
        out[g] = {
          category: cat,
          testid: `${prefix}-extra-${label.replace(/ /g, "-")}`,
        };
    }
  }
  ...
  if (lang === "java") {
    for (const [g, label] of JAVA_COMPOUND_LIST) {
      if (!out[g])
        out[g] = {
          category: cat,
          testid: `${prefix}-compop-${label.replace(/ /g, "-")}`,
        };
    }
    for (const id of JAVA_IDIOMS_LIST) {
      if (!out[id])
        out[id] = { category: cat, testid: `${prefix}-idiom-${slug(id)}` };
    }
...
function lookupInCategory(
  glyph: string,
  cat: KeyRef["category"],
): KeyRef | null {
  switch (cat) {
    case "main":
      return MAIN[glyph] ?? null;
    case "adv-math":
      return ADV_MAP[glyph] ?? null;
    case "letters":
      return LTR_MAP[glyph] ?? null;
    case "programming-python":
      return PY_MAP[glyph] ?? null;
    case "programming-java":
      return JAVA_MAP[glyph] ?? null;
    default:
      return GRID_MAPS[cat]?.[glyph] ?? null;
  }
}
...
export function glyphsForCategory(cat: KeyRef["category"]): string[] {
  switch (cat) {
    ...
    case "adv-math":
      return Object.keys(ADV_MAP);
    case "letters":
      return Object.keys(LTR_MAP);
    case "programming-python":
      return Object.keys(PY_MAP);
    case "programming-java":
      return Object.keys(JAVA_MAP);
    default:
      return Object.keys(GRID_MAPS[cat] ?? {});
  }
}
