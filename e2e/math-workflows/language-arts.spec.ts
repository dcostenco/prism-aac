...
test.describe("language-arts workflow", () => {
  ...
  test('POS-tag "the cat ran fast"', async ({ page }, ti) => {
    await runProblem(page, ti, ["t=ART", "c=N", "r=V", "f=ADV"], CATEGORY);
  });
  ...
  test('POS-tag "she gave him a red book"', async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["s=PRON", "g=V", "h=PRON", "a=ART", "r=ADJ", "b=N"],
      CATEGORY,
    );
  });

  test('sentence-type ID — "go home now."', async ({ page }, ti) => {
    await runProblem(page, ti, ["g=V", "h=N", "t=IMP"], CATEGORY);
  });

  test('sentence-type ID — "are you ready?"', async ({ page }, ti) => {
    await runProblem(page, ti, ["a=AUX", "y=PRON", "r=ADJ", "t=INT"], CATEGORY);
  });
  ...
  test('compound sentence — "he ran and she walked"', async ({ page }, ti) => {
    await runProblem(
      page,
      ti,
      ["h=PRON", "r=V", "a=CONJ", "s=PRON", "w=V", "t=COMP"],
      CATEGORY,
    );
  });
});
