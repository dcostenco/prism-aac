...
test.describe("Eval button visibility per domain", () => {
  ...
  test("🧮 Eval is visible on programming-python chip", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
  ...
  test("🧮 Eval is visible on programming-java chip", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
  ...
  test("🧮 Eval is HIDDEN on chemistry chip", async ({ page, baseURL }) => {
    ...
    await expect(page.locator('[data-testid="math-tutor-eval"]')).toHaveCount(
      0,
    );
  });
...
test.describe("Debug button visibility per domain", () => {
  test("🐛 Debug is visible on programming-python chip", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    await pickCategory(page, "programming-python");
    await expect(
      page.locator('[data-testid="math-tutor-debug"]'),
    ).toBeVisible();
  });

  test("🐛 Debug is HIDDEN on programming-java chip (no in-browser JVM)", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    await pickCategory(page, "programming-java");
    await expect(page.locator('[data-testid="math-tutor-debug"]')).toHaveCount(
      0,
    );
  });
  ...
  test("switching python → math hides Debug, switching back shows it", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
...
test.describe("Math 🧮 Eval — happy path (mathjs, no network)", () => {
  test('typing 2 + 3 then tapping Eval renders "= 5" in the overlay', async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="math-tutor-response"]'),
    ).toHaveAttribute("data-mode", "eval");
  });

  test("typing nothing and tapping Eval is a no-op (no overlay)", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
...
test.describe("Programming chip → typeable identifiers (the surgical fix)", () => {
  test("python letters row exposes shift + page-toggle + 16 letters", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    await pickCategory(page, "programming-python");
    await expect(
      page.locator('[data-testid="math-python-letters-row"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-letters-shift"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-letters-page-toggle"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-ltr-a"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-ltr-p"]'),
    ).toBeVisible();
    // q-z page should be reachable via toggle.
    await page
      .locator('[data-testid="math-python-letters-page-toggle"]')
      .click();
    await expect(
      page.locator('[data-testid="math-python-ltr-q"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-ltr-z"]'),
    ).toBeVisible();
  });
  ...
  test("python digit + underscore row is rendered", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    await pickCategory(page, "programming-python");
    await expect(
      page.locator('[data-testid="math-python-digit-0"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-digit-9"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-python-underscore"]'),
    ).toBeVisible();
  });

  test("java letters row + digits + underscore are rendered", async ({
    page,
    baseURL,
  }) => {
    await gotoDev(page, baseURL);
    await pickCategory(page, "programming-java");
    await expect(
      page.locator('[data-testid="math-java-letters-row"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="math-java-ltr-a"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="math-java-digit-0"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-java-underscore"]'),
    ).toBeVisible();
  });
});
