...
test.describe("MathTutorTool (Phase 5C)", () => {
  test("three tutor buttons (Hint / Check / Solve) render in the math header", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="math-tutor-check"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="math-tutor-solve"]'),
    ).toBeVisible();
  });

  test("tutor buttons are enabled regardless of auth state (Phase 5D — gate removed)", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="math-tutor-signed-out"]'),
    ).toHaveCount(0);
  });

  test("all 3 tutor buttons meet the 44px tap-target floor", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
