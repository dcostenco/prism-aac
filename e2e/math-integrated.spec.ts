...
async function bootClean(page: Page, baseURL: string | undefined) {
  ...
  await page.evaluate(() => {
    try {
      ...
      const cur = {
        state: { toolbarConfig: { order: [], enabled: { math: true } } },
        version: 0,
      };
...
test.describe("MathPanel integrated (Phase 4)", () => {
  test("opening Math mounts the new cell-grid canvas + keyboard region + lock tool", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="math-keyboard-region"]'),
    ).toBeVisible();
  ...
  test("typing 5 + 7 in the new keyboard fills three grid cells", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
    ...
    const glyphs = await page
      ...
      .count();
    expect(glyphs, '3 grid glyphs after typing "5 + 7"').toBeGreaterThanOrEqual(
      3,
    );
  });

  test("Done sends serialized expression to MessageBar and closes Math", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
    ...
    const msgBarText = await page
      .locator("[data-messaging-mode]")
      .first()
      .innerText();
    expect(msgBarText, "serialized expression appended to MessageBar").toMatch(
      /5\s+.*7/,
    );
  });

  test("Close discards the grid (no MessageBar update)", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
  ...
  test("Adv. Math chip works inside the integrated panel", async ({
    page,
    baseURL,
  }) => {
    await bootClean(page, baseURL);
    ...
    const decorations = await page
      .locator('[data-testid="math-grid-decorations"] line')
      .count();
    expect(decorations, "fraction-bar decoration drawn").toBeGreaterThanOrEqual(
      1,
    );
  });
});
