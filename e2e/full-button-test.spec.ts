...
for (const device of DEVICES) {
  test.describe(`${device.name} — full button test`, () => {
    ...
    const vis = (s: string) => page.locator(s).first().isVisible({ timeout: 2000 }).catch(() => false);
    const clk = async (s: string) => {
      const l = page.locator(s).first();
      if (await l.isVisible({ timeout: 2000 }).catch(() => false)) { await l.click({ timeout: 3000 }); return true; }
      return false;
    };
    const close = async () => {
      await page.locator('[aria-label*="close" i]').first().click({ timeout: 2000 }).catch(async () => { await page.mouse.click(10, 10); });
      await page.waitForTimeout(500);
    };

    test('01. Toolbar visible', async () => { expect(await vis('[data-testid="aac-toolbar-strip"]')).toBe(true); });
    test('02. Prediction tiles visible', async () => { expect(await vis('button[aria-label*="Predict:"]')).toBe(true); });
    test('03. Keyboard visible', async () => { expect(await vis('button[data-key="H"]')).toBe(true); });
    test('04. Speak button visible', async () => { expect(await vis('button:has-text("Speak")')).toBe(true); });
    test('05. Language selector visible', async () => { expect(await vis('[data-testid="language-button-input"]')).toBe(true); });

    test('06. Keyboard hide/show toggle', async () => {
      const kbBtn = page.locator('button:has-text("Keyboard"), button:has-text("Hide KB")').first();
    ...
    test('07. Type HI and Speak enabled', async () => {
      ...
      const disabled = await page.locator('button:has-text("Speak")').last().isDisabled();
    ...
    test('20. Auto button', async () => { expect(await clk('button:has-text("Auto")')).toBe(true); });
    test('21. Delete/Clear', async () => { expect(await clk('button[aria-label*="Delete"], button[aria-label*="Clear"]')).toBe(true); });
