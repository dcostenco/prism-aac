...
test.describe("1. Pyodide cold-start — Python eval works against the live runtime", () => {
  test('typing print(1+2) in Python chip and tapping Eval prints "3"', async ({
    page,
    baseURL,
  }, info) => {
    ...
    await page.screenshot({
      path: `test-results/verify-pyodide-${info.project.name}-typed.png`,
      fullPage: false,
    });
    ...
    ).toBeVisible({ timeout: PYODIDE_TIMEOUT });
    ...
    await page.screenshot({
      path: `test-results/verify-pyodide-${info.project.name}-result.png`,
      fullPage: false,
    });
    const text = await page
      .locator('[data-testid="math-tutor-response"]')
      .textContent();
...
test.describe('2. Java Eval Piston roundtrip — System.out.println(1+1) prints "2"', () => {
  test("java chip evaluates a print expression via Piston", async ({
    page,
    baseURL,
  }, info) => {
    ...
    await page.screenshot({
      path: `test-results/verify-java-${info.project.name}-typed.png`,
      fullPage: false,
    });
    await evalBtn.click();
    ...
    await page.screenshot({
      path: `test-results/verify-java-${info.project.name}-result.png`,
      fullPage: false,
    });
    const text =
      (await page
...
test.describe("3. PDF Reader panel renders", () => {
  test("chip opens the PDF Reader panel with the empty-state prompt", async ({
    page,
    baseURL,
  }, info) => {
    ...
    await page.screenshot({
      path: `test-results/verify-pdf-${info.project.name}.png`,
      fullPage: false,
    });
  });
});
...
test.describe("4. OCR Capture panel renders", () => {
  test("chip opens the OCR panel with the empty-state prompt", async ({
    page,
    baseURL,
  }, info) => {
    await gotoApp(page, baseURL);
    ...
    await expect(
      page.locator('[data-testid="ocr-capture-pick"]'),
    ).toBeVisible();
    await page.screenshot({
      path: `test-results/verify-ocr-${info.project.name}.png`,
      fullPage: false,
    });
  });
});
...
test.describe("5. Word-by-word highlight cycles during Speak", () => {
  test("tapping Speak with a multi-word message lights up at least 2 distinct words", async ({
    page,
    baseURL,
  }, info) => {
    await gotoApp(page, baseURL);
    ...
    await page.evaluate(() => {
      try {
        window.localStorage.setItem(
          "prism-aac-message",
          JSON.stringify({
            state: { soundEnabled: true, autoSpeak: true },
            version: 3,
          }),
        );
      } catch {
    ...
    const speakBtn = page
      .locator('button[aria-label*="speak" i]')
      .filter({ hasText: "▶" })
      .first();
    if ((await speakBtn.count()) === 0) {
      console.log(
        "[verify-highlight] Speak button not found by label — using fallback selector",
      );
    }
    ...
    const samples = await page.evaluate(async () => {
      ...
      const speak = document.querySelector(
        "button.aac-speak",
    ...
    await page.screenshot({
      path: `test-results/verify-highlight-${info.project.name}.png`,
      fullPage: false,
    });
    const distinctActive = new Set(samples.filter((s) => s.length > 0));
    console.log(
      `[verify-highlight] distinct active words across 3 s: ${[...distinctActive].join(" | ")}`,
    );
