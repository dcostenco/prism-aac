...
test.describe("MathMainKeyboard (Phase 1B)", () => {
  test("keyboard mounts with all expected keys", async ({ page, baseURL }) => {
    ...
    for (const op of [
      ...
      await expect(
        page.locator(`[data-testid="math-key-${op}"]`),
      ).toBeVisible();
    }
    ...
    await expect(
      page.locator('[data-testid="math-key-backspace"]'),
    ).toBeVisible();
  });

  test("every digit key commits the matching glyph and advances the cursor", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("plus / minus / times / divide / equals commit their glyphs", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("return key drops cursor to next row at column 0", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
  ...
  test("all keys meet the 44px minimum tap-target floor (AAC accessibility)", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    const keys = await page.locator('[data-testid^="math-key-"]').all();
    expect(keys.length, "keyboard exposes ≥ 20 keys").toBeGreaterThanOrEqual(
      20,
    );
    for (const k of keys) {
  ...
  test("typing a 4-glyph expression renders 4 SVG glyphs in the grid", async ({
    page,
    baseURL,
  }) => {
    await gotoDevPage(page, baseURL);
    ...
    expect(
      glyphCount,
      "4 glyph nodes rendered after typing 5 + 7 =",
