...
test.describe('Daily AAC Flow — typical day session', () => {
  test.beforeEach(async ({ page, baseURL }) => {
    ...
    await page.evaluate(() => {
      try { localStorage.clear(); sessionStorage.clear(); } catch { /* sandboxed */ }
    });
  ...
  test('complete daily flow — morning to evening AAC usage', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: `test-results/daily-02-morning-${proj}.png` });
    ...
    await page.screenshot({ path: `test-results/daily-03-prediction-${proj}.png` });
    ...
    await page.waitForSelector('[aria-label="Home vocabulary board"]', { timeout: 12_000 });
    ...
    await page.screenshot({ path: `test-results/daily-04-categories-home-${proj}.png` });
    ...
    if (await foodFolder.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (await helloTile.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await page.screenshot({ path: `test-results/daily-05-phrase-tile-${proj}.png` });
    }
    ...
    await catToolbarBtn.click();
    await page.waitForTimeout(300);
    if (!(await page.locator('button[data-key="Q"]').isVisible().catch(() => false))) {
      await catToolbarBtn.click();
      await page.waitForTimeout(300);
    }
    ...
    if (!(await page.locator('button[data-key="Q"]').isVisible().catch(() => false))) {
      ...
      if (await kbCycle.isVisible({ timeout: 1000 }).catch(() => false)) {
        await kbCycle.click();
        await page.waitForTimeout(300);
      }
    }
    await expect(page.locator('button[data-key="Q"]')).toBeVisible({ timeout: 5000 });
    ...
    if (await periodKey.isVisible({ timeout: 1000 }).catch(() => false)) {
      await periodKey.click();
      await page.locator('button[data-action="space"]').click();
      const questionKey = page.locator('button[data-key="?"]').first();
      if (await questionKey.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      if (await periodInSymbols.isVisible({ timeout: 1500 }).catch(() => false)) {
    ...
    await page.screenshot({ path: `test-results/daily-06-punctuation-${proj}.png` });
    ...
    if (await aiToolbarBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await aiToolbarBtn.click();
      await page.waitForTimeout(800);

      // Panel always renders as expanded when sidePanel === 'ai-chat'.
      const aiPanel = page.locator('[data-testid="ai-chat-panel"][data-state="expanded"]');
      ...
      await page.screenshot({ path: `test-results/daily-08-aichat-${proj}.png` });
    ...
    await expect(page.getByText(/Categories|Grid|Theme|Voice/i).first()).toBeVisible({ timeout: 5000 });

    await page.screenshot({ path: `test-results/daily-09-settings-${proj}.png` });
    ...
    if (await settingsClose.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await page.locator('div[role="dialog"]').first().click({ position: { x: 5, y: 5 } });
    }
    ...
    // =====================================================================
    const historyToolbarBtn = page.getByRole('button', { name: 'History' }).first();
    if (await historyToolbarBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await historyToolbarBtn.click();
      await page.waitForTimeout(500);

      // History modal uses role="dialog" aria-modal="true" with aria-labelledby="history-modal-title".
      const historyDialog = page.locator('div[role="dialog"][aria-modal="true"]');
      if (await historyDialog.isVisible({ timeout: 3000 }).catch(() => false)) {
        await page.screenshot({ path: `test-results/daily-10-history-${proj}.png` });

        // Close via the close button (aria-label from t('close_history')).
        const histClose = page.locator('div[role="dialog"] button[aria-label]').first();
        if (await histClose.isVisible({ timeout: 1500 }).catch(() => false)) {
    ...
    // =====================================================================
    const scheduleToolbarBtn = page.getByRole('button', { name: 'Schedule' }).first();
    if (await scheduleToolbarBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const scheduleVisible = await page.getByText(/First|Then|Schedule|Reset Day/i).first()
        .isVisible({ timeout: 3000 }).catch(() => false);

      if (scheduleVisible) {
        await page.screenshot({ path: `test-results/daily-11-schedule-${proj}.png` });
      }
    ...
    if (await gamesToolbarBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await gamesToolbarBtn.click();
      await page.waitForTimeout(600);

      // Games panel shows a grid of colorful game cards with titles.
      const gamesVisible = await page.getByText(/Bubble Pop|Color Hunt|Match It|My Story|Yes.*No|Finish It/i).first()
        .isVisible({ timeout: 3000 }).catch(() => false);

      if (gamesVisible) {
        await page.screenshot({ path: `test-results/daily-12-games-${proj}.png` });
      }
    ...
    // 15. Toggle sound
    // =====================================================================
    const soundToolbarBtn = page.getByRole('button', { name: /Sound/i }).first();
    if (await soundToolbarBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    await page.screenshot({ path: `test-results/daily-13-post-alert-${proj}.png` });
    ...
    if (await langBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await page.screenshot({ path: `test-results/daily-14-language-picker-${proj}.png` });
      ...
      if (await langPicker.isVisible().catch(() => false)) {
    ...
    const qVisibleInSymbols = await page.locator('button[data-key="Q"]').isVisible().catch(() => false);
    expect(qVisibleInSymbols).toBe(false);

    await page.screenshot({ path: `test-results/daily-15-symbols-${proj}.png` });
    ...
    await expect(page.locator('button[data-key="Q"]')).toBeVisible({ timeout: 3000 });
...
test.describe('Resilience — session restore', () => {
  test('settings survive page reload', async ({ page, baseURL }) => {
    ...
    if (await soundBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (soundWasVisible) {
      ...
      if (await soundAfter.isVisible({ timeout: 2000 }).catch(() => false)) {
  ...
  test('word frequency store persists across reload', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('category store persists across reload', async ({ page, baseURL }) => {
    ...
    await expect(page.getByText(/Hello|Goodbye|Thank you/i).first()).toBeVisible({ timeout: 5000 });
  ...
  test('schedule panel opens and shows First-Then board after reload', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });

    const schedBtn = page.getByRole('button', { name: /Schedule/i }).first();
    if (!(await schedBtn.isVisible({ timeout: 2000 }).catch(() => false))) return;

    // Open schedule, verify it renders.
    await schedBtn.click();
    await page.waitForTimeout(600);
    const boardText = page.getByText(/First|Then|Schedule|Reset Day|Add/i).first();
...
test.describe('Resilience — crash recovery (ErrorBoundary)', () => {
  test('ErrorBoundary renders emergency AAC with input + word buttons', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    await page.evaluate(() => {
      ...
      const crashEl = document.querySelector('[data-testid="aac-toolbar-strip"]');
      if (crashEl) {
    ...
    const kbVisible = await keyboard.isVisible().catch(() => false);
    const emergencyVisible = await emergencyInput.isVisible().catch(() => false);
    ...
    if (emergencyVisible) {
      ...
      const words = ['Help', 'Yes', 'No', 'Stop', 'Bathroom', 'Water', 'Hungry', 'Pain'];
      let found = 0;
      for (const w of words) {
        if (await page.locator(`button:has-text("${w}")`).isVisible().catch(() => false)) found++;
      }
      expect(found, `Expected ≥6 emergency word buttons, found ${found}`).toBeGreaterThanOrEqual(6);
      ...
      expect(kbVisible, 'App should either show keyboard or emergency mode').toBe(true);
    }

    // Clean up corrupted storage.
    await page.evaluate(() => {
      try { localStorage.removeItem('prism-aac-settings'); } catch { /* ignore */ }
    });
  });

  test('app recovers from corrupted message store', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('app recovers from corrupted prediction store', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
...
test.describe('Resilience — main thread freeze & hang simulation', () => {
  test('UI recovers after 3-second main thread block', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('UI recovers after 5-second setTimeout starvation', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    await page.evaluate(() => {
      ...
      function flood() {
        ...
        while (Date.now() < end) { /* 10ms sync work per tick */ }
    ...
    await expect(page.locator('button[data-key="Q"]')).toBeVisible({ timeout: 10_000 });
    await page.locator('button[data-key="H"]').click();
  ...
  test('rapid panel switching does not crash', async ({ page, baseURL }) => {
    ...
    await page.evaluate(() => {
      ...
      try {
        ...
        if (uiStore) {
          uiStore.setState({ showSettings: false, showHistory: false, sidePanel: 'none' });
        }
      } catch {}
      ...
      dialogs.forEach((d) => {
        ...
        btns.forEach((b) => {
          if (b.textContent?.trim() === '✕' || b.textContent?.trim() === '×') b.click();
        });
      });
    });
    await page.waitForTimeout(500);
    ...
    for (const name of panelButtons) {
      if (await page.locator('button[data-key="Q"]').isVisible().catch(() => false)) break;
      const btn = page.getByRole('button', { name: new RegExp(name, 'i') }).first();
    ...
    const consoleCrashes = await page.evaluate(() => !!document.querySelector('#emergency-input'));
    expect(consoleCrashes, 'ErrorBoundary should not have activated').toBe(false);

    // Toolbar should still be visible (app is alive even if keyboard is behind a panel).
    await expect(page.locator('[data-testid="aac-toolbar-strip"]')).toBeVisible({ timeout: 5000 });
  });
...
test.describe('Resilience — network & offline simulation', () => {
  test('app boots and types in offline mode', async ({ page, baseURL, context }) => {
    const start = baseURL || '/';
  ...
  test('app survives network toggle mid-session', async ({ page, baseURL, context }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
...
test.describe('Resilience — visibility & tab lifecycle', () => {
  test('app state persists through visibility hidden/visible cycle', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
...
test.describe('Resilience — localStorage edge cases', () => {
  test('app boots with completely empty localStorage', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('app handles localStorage quota exceeded gracefully', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    // No ErrorBoundary crash.
    const crashed = await page.evaluate(() => !!document.querySelector('#emergency-input'));
    expect(crashed).toBe(false);

    // Clean up junk.
    await page.evaluate(() => {
      for (let i = 0; i < 10; i++) {
        try { localStorage.removeItem(`__junk_${i}`); } catch { /* ignore */ }
      }
    });
  });

  test('app handles all stores corrupted simultaneously', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    await page.evaluate(() => {
      ...
      for (const key of stores) {
        try { localStorage.setItem(key, '!!!GARBAGE!!!'); } catch { /* ignore */ }
      }
    });
    ...
    const kbOk = await keyboard.isVisible().catch(() => false);
    const emergencyOk = await emergency.isVisible().catch(() => false);
...
test.describe('Resilience — long session drift', () => {
  test('prediction quality does not degrade after 50+ interactions', async ({ page, baseURL }) => {
    test.setTimeout(120_000);
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const words = [
      'hello', 'water', 'food', 'please', 'thank', 'more', 'yes', 'no',
      'help', 'stop', 'go', 'want', 'like', 'good', 'bad', 'happy',
      'sad', 'tired', 'play', 'read',
    ];
    ...
    const preds = await page.locator('button[aria-label^="Predict:"]').allInnerTexts();
    expect(preds.length, 'Prediction bar should still show tiles after 20-word session').toBeGreaterThan(0);
    ...
    expect(hasPrefix, `Expected a "hel*" prediction, got: ${JSON.stringify(preds)}`).toBe(true);
  });

  test('memory does not leak during repeated panel open/close', async ({ page, baseURL }) => {
...
test.describe('Tier 1 — Emergency & panic', () => {
  test('emergency phrase detection triggers countdown modal', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const appeared = await emergencyDialog.isVisible({ timeout: 8000 }).catch(() => false);
    ...
    if (appeared) {
      ...
      const cancelBtn = emergencyDialog.locator('button:has-text("Cancel")').first();
  ...
  test('panic stop — 3× Escape kills all activity', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const alive = await page.locator('[data-testid="aac-toolbar-strip"]').isVisible().catch(() => false);
    expect(alive).toBe(true);
  });

  test('physical keyboard input — Enter speaks, Backspace deletes', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('concurrent speech — rapid Speak spam does not crash', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
...
test.describe('Tier 2 — Translation, history, tones, search', () => {
  test('history modal — tap entry re-fills message bar', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    if (await histBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await histDialog.isVisible({ timeout: 3000 }).catch(() => false)) {
        // Find and tap the history entry.
        const entry = histDialog.locator('button:has-text("history test phrase")').first();
  ...
  test('tone selector — 9 tones + auto visible', async ({ page, baseURL }) => {
    ...
    if (await toneBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const hasTones = await autoTone.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('category search — find phrases across all categories', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const searchBtn = page.locator('button:has-text("SEARCH"), button:has-text("Search")').first();
    if (await searchBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (await searchInput.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const results = page.locator('button:has-text("Water"), button:has-text("water")');
        const count = await results.count();
        expect(count).toBeGreaterThan(0);
      }

      // Close search.
      const closeSearch = page.locator('button:has-text("✕"), button[aria-label*="close" i]').first();
      if (await closeSearch.isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test('bedside mode — opens overlay with mic and quick phrases', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const aiBtn = page.getByRole('button', { name: 'AI' }).first();
    if (!(await aiBtn.isVisible({ timeout: 2000 }).catch(() => false))) return;
    ...
    if (!(await bedsideBtn.isVisible({ timeout: 3000 }).catch(() => false))) return;
    ...
    await expect(page.locator('[data-testid="bedside-cards-section"]')).toBeVisible();

    // Big mic button should be present.
    const micBtn = overlay.locator('button[aria-label*="listening" i], button[aria-label*="Start" i]').first();
    await expect(micBtn).toBeVisible();

    // Add card button should be present.
    await expect(page.locator('[data-testid="bedside-add-card-btn"]')).toBeVisible();
  ...
  test('caregiver notes panel — add and view a note', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    if (!(await notesBtn.isVisible({ timeout: 2000 }).catch(() => false))) return;

    await notesBtn.click();
    await page.waitForTimeout(600);

    // Notes panel should have an author input and note textarea.
    const authorInput = page.locator('input[placeholder*="role" i], input[placeholder*="author" i]').first();
  ...
  test('delete short-press removes one word, not all', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('translation row appears when input ≠ output language', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const hasTranslation = await translationRow.isVisible({ timeout: 3000 }).catch(() => false);

    // Translation may not fire without AI backend — just verify no crash.
    // The output language selector should show ES.
    const outputLang = page.locator('[data-testid="language-button-output"], [data-testid="language-button-output-mismatch"]').first();
    if (await outputLang.isVisible({ timeout: 2000 }).catch(() => false)) {
...
test.describe('Tier 3 — Settings deep dive', () => {
  test('grid size change — tiles resize', async ({ page, baseURL }) => {
    ...
    if (await size4.isVisible({ timeout: 2000 }).catch(() => false)) {
      await size4.click();
      await page.waitForTimeout(300);
    }
    ...
    const tiles = page.locator('button[class*="PhraseTile"], [class*="phrase"]').first();
    if (await tiles.isVisible({ timeout: 2000 }).catch(() => false)) {
  ...
  test('custom phrase section exists in settings', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const customSection = page.getByText(/Custom Categories|Custom Phrases|Add.*phrase/i).first();
    const found = await customSection.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('high contrast toggle', async ({ page, baseURL }) => {
    ...
    const hcToggle = page.locator('button:has-text("High Contrast"), label:has-text("High Contrast")').first();
    if (await hcToggle.isVisible({ timeout: 3000 }).catch(() => false)) {
  ...
  test('PIN pad locks settings access', async ({ page, baseURL }) => {
    ...
    const hasPinPad = await pinDigit.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('export/import settings roundtrip', async ({ page, baseURL }) => {
    ...
    const exported = await page.evaluate(() => {
      ...
      for (const key of ['prism-aac-settings', 'prism-aac-categories', 'prism-aac-predictions']) {
...
test.describe('Tier 4 — Games', () => {
  test('Bubble Pop game — launches and shows bubbles', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });

    const gamesBtn = page.getByRole('button', { name: /Games/i }).first();
    if (!(await gamesBtn.isVisible({ timeout: 2000 }).catch(() => false))) return;
    ...
    if (await bubblePop.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const hasScore = await score.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('Color Hunt game — launches and shows colors', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    if (await colorHunt.isVisible({ timeout: 3000 }).catch(() => false)) {
  ...
  test('My Story game — launches and shows cards', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    if (await myStory.isVisible({ timeout: 3000 }).catch(() => false)) {
      await myStory.click();
      await page.waitForTimeout(1000);
...
test.describe('Tier 4 — Accessibility', () => {
  test('focus trap in Settings modal — Tab stays inside', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('keyboard-only navigation — Tab through toolbar', async ({ page, baseURL }, testInfo) => {
  ...
  test('aria-live regions exist for dynamic content', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
...
test.describe('Tier 4 — Cross-component edge cases', () => {
  test('mid-speech panel switch does not crash', async ({ page, baseURL }) => {
    ...
    const gamesBtn = page.getByRole('button', { name: /Games/i }).first();
    if (await gamesBtn.isVisible({ timeout: 500 }).catch(() => false)) {
      await gamesBtn.click();
      await page.waitForTimeout(300);
    ...
    await page.waitForTimeout(300);
    await page.locator('button[aria-label*="close" i]').first().click().catch(() => {});
    await page.waitForTimeout(300);
  ...
  test('greeting banner shows and can be dismissed', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    ...
    const greeting = page.locator('text=/Good (morning|afternoon|evening|night)/i').first();
    const bannerVisible = await greeting.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    if (bannerVisible) {
      ...
      if (await dismissBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        await page.reload({ waitUntil: 'domcontentloaded' });
        await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
        const reappeared = await greeting.isVisible({ timeout: 2000 }).catch(() => false);
        expect(reappeared).toBe(false);
      }
    }
  });

  test('marketplace panel opens and shows catalog', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });

    const mpBtn = page.getByRole('button', { name: /Marketplace/i }).first();
    if (!(await mpBtn.isVisible({ timeout: 2000 }).catch(() => false))) return;

    await mpBtn.click();
    await page.waitForTimeout(600);

    // Should see tab buttons (All, Vocabulary, Games, Tools).
    const tabs = page.locator('button:has-text("All"), button:has-text("Vocabulary")');
  ...
  test('comfort player panel opens with empty state', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    await page.evaluate(() => {
      try {
        const raw = localStorage.getItem('prism-aac-settings');
        const data = raw ? JSON.parse(raw) : { state: { toolbarConfig: { order: [], enabled: {} } }, version: 0 };
        if (!data.state.toolbarConfig) data.state.toolbarConfig = { order: [], enabled: {} };
    ...
    if (await cpBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const closeBtn = page.locator('button[aria-label="Close comfort player"]');
  ...
  test('sentence-end auto-speak — period triggers speech', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('autocorrect suggestion bar appears and is tappable', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const hasCorrection = await suggestion.isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('Regression — PDF/OCR reader fixes', () => {
  test('PDF reader hides keyboard when panel is expanded', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    if (!(await pdfBtn.isVisible({ timeout: 3000 }).catch(() => false))) return;
    ...
    const kbVisible = await page.locator('button[data-key="Q"]').isVisible().catch(() => false);
    expect(kbVisible, 'Keyboard should be hidden when PDF reader is open').toBe(false);
  ...
  test('OCR capture hides keyboard when panel is expanded', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
    ...
    const ocrBtn = page.locator('button[aria-label="Screenshot reader (OCR)"]').first();
    if (!(await ocrBtn.isVisible({ timeout: 3000 }).catch(() => false))) return;
    ...
    const kbVisible = await page.locator('button[data-key="Q"]').isVisible().catch(() => false);
    expect(kbVisible, 'Keyboard should be hidden when OCR capture is open').toBe(false);
  ...
  test('OCR panel has Stop button data-testid when speaking', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
  ...
  test('PDF reader has Stop button in header when speaking', async ({ page, baseURL }) => {
    const start = baseURL || '/';
    await page.goto(start);
    await page.waitForSelector('button[data-key="Q"]', { timeout: 30_000 });
