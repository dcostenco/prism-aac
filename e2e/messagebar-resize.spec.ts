...
interface BBox { width: number; height: number; }
...
test.describe('MessageBar resize — bbox + screenshot per resolution', () => {
  test('measures + screenshots message bar', async ({ page, baseURL }, info) => {
    ...
    await page.screenshot({
      ...
      clip: { x: 0, y: 0, width: page.viewportSize()!.width, height: Math.min(500, page.viewportSize()!.height) },
    });

    // Print the measurement BEFORE any assertion so it shows up in console
    // for the failure report too.
    console.log(`[${info.project.name}] viewport=${page.viewportSize()!.width}x${page.viewportSize()!.height}  wrapper=${wrapper?.height ?? 'null'}px  text=${textArea?.height ?? 'null'}px`);
